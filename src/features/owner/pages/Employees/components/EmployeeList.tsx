...
export const EmployeeList: React.FC<EmployeeListProps> = ({ employees }) => {
  ...
  return (
    <>
      {employees && (
        <div className="mb-30">
          {employees.filter((data) => data.account.status == "Pengelola Gudang")
            .length != 0 && (
            <Divider my="xs" label="Pengelola Gudang" labelPosition="left" />
          )}
          {employees
            .filter((data) => data.account.status == "Pengelola Gudang")
            .map((data, index) => (
              <section
                onClick={() =>
                  navigate("/employee-data/detail", { state: { data } })
          ...
          {employees.filter((data) => data.account.status == "Pengelola Frozen")
            .length != 0 && (
            <Divider my="xs" label="Pengelola Frozen" labelPosition="left" />
          )}
          {employees
            .filter((data) => data.account.status == "Pengelola Frozen")
            .map((data, index) => (
              <section
                onClick={() =>
                  navigate("/employee-data/detail", { state: { data } })
          ...
          {employees.filter((data) => data.account.status == "Part time")
            .length != 0 && (
            <Divider my="xs" label="Part time" labelPosition="left" />
          )}
          {employees
          ...
          {employees.filter((data) => data.account.status == "Tidak Aktif")
            .length != 0 && (
            <Divider my="xs" label="Tidak Aktif" labelPosition="left" />
          )}
          {employees
            .filter((data) => data.account.status == "Tidak Aktif")
            .map((data, index) => (
              <section
                onClick={() =>
                  navigate("/employee-data/detail", { state: { data } })
              ...
              >
                <div
                  className="bg-slate-100 shadow-md rounded-2xl p-3"
                  style={{ marginTop: "5px" }}
                >
                  <div className="grid grid-cols-12 pl-2">
                    <div className="col-span-1 my-auto">
