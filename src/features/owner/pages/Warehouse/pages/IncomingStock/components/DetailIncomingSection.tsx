...
import {
  useDeleteIncomingData,
  useDeleteIncomingDetail,
...
interface DetailIncomingProps {
  incomingData: IncomingDataType;
  details: DetailIncomingType[];
  RefetchDetails: () => void;
}

export const DetailIncomingSection: React.FC<DetailIncomingProps> = ({
  incomingData,
  details,
  RefetchDetails,
}) => {
  ...
  const deleteIncomingDataMutation = useDeleteIncomingData();
  const deleteIncomingData = async (id: number | undefined) => {
    deleteIncomingDataMutation.mutateAsync(id, {
      onSuccess: (data) => {
  ...
  const deleteDetailMutation = useDeleteIncomingDetail();
  const deleteDetail = async (id: number | undefined) => {
    deleteDetailMutation.mutateAsync(id, {
      onSuccess: () => {
        RefetchDetails();
        showNotification({
          message: "Detail barang masuk berhasil dihapus",
          color: "green",
          position: "top-center",
        });
        closeDrawer();
  ...
  //   END FOR DELETE INCOMING DETAIL

  //   UPDATE INCOMING DETAIL
  ...
  //   END FOR UPDATE INCOMING DETAIL
  return (
    <>
      <div className="bg-white shadow-md rounded-md p-4">
        <div className="grid grid-cols-12">
          <div className="col-span-6">
            <Text size="xs" fw={400}>
              {format(incomingData.created_at, "EEEE, dd MMM yyyy HH:mm", {
                locale: id,
              })}
            </Text>
          </div>
          <div className="col-span-6 flex justify-end">
            <Image
              ...
              w={20}
              src={
                incomingData.employee.profile_pic
                  ? `${BaseURL}/uploads/employees/${incomingData.employee?.profile_pic}`
                  : DEFAULT_IMAGE
              }
            />
            <Text size="xs" fw={400} truncate="end">
              {incomingData.employee.name}
            </Text>
          </div>
        </div>
        <Divider my={8} />
        ...
        <div className="px-1">
          <div className=" text-center px-2">
            ...
            <Text size="sm" c={"red"} fw={700}>
              ...
              || {incomingData.details.length} Barang "
            </Text>
          </div>
          <div className="flex justify-betwen gap-2 mt-2 mb-2">
            ...
            <Button
              ...
              fullWidth
              onClick={() => {
                deleteIncomingData(incomingData?.id);
              }}
            >
              Ya! Hapus
            </Button>
